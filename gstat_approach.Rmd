...
```{r}
#Load necessary R packages. Note: gstat and sgeostat may interfere with each other and produce errors. If this is the case, only run one of these packages at a time for the necessary functions and restart R session between loading each respective package
library(gstat)
#library(sgeostat)
library(rgl)
library(sp)
...
#Find the maximum distance that should be included in analysis
...
plot(Variogram(park_gls, form= ~ Utm_E + Utm_N, maxDist=64308.87))
plot(Variogram(park_sp, form= ~ Utm_E + Utm_N, maxDist=64308.87))
plot(Variogram(park_sp_nug, form= ~ Utm_E + Utm_N, maxDist=64308.87))


#Make variogram with periodicity
...
m3 <- fit.wave(est_var, 0, 294462.1, 31832, type='c', iterations=10, tolerance=1e-06, echo=FALSE, plot.it=FALSE, weighted=TRUE)
